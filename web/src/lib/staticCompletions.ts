// Worker-free fallback completions for HTML/CSS/JS. Monaco's own built-in language services for
// these three run inside a Web Worker, which some hosting environments' CSP blocks entirely
// (no `worker-src blob:` allowance). These static lists work regardless — same philosophy as the
// Jedi-backed Python provider in sandbox.ts, just without real semantic inference.
